...
class precompiled_wheel_utils:
    ...
    def is_rocm_system() -> bool:
        ...
        forced_target = os.getenv("VLLM_TARGET_DEVICE", VLLM_TARGET_DEVICE).lower()
        if forced_target == "cuda":
            return False
        if forced_target == "rocm":
            return True
    ...
    def determine_wheel_url() -> tuple[str, str | None]:
        ...
        # Respect explicit or auto-detected target first.
        # This avoids misrouting CUDA environments on clusters that expose
        # ROCm markers such as /opt/rocm or rocminfo.
        if forced_target == "rocm":
