...
@pytest.mark.parametrize("num_engines", [1, 4])
def test_partial_vector_metric(test_registry, num_engines):
    c = prometheus_client.Counter(
        "vllm:spec_decode_num_partial_accepted_tokens_per_pos",
        "Vector-like partial counter metric",
        labelnames=["position", "model", "engine_index"],
        registry=test_registry,
    )
    for i in range(num_engines):
        c.labels(position="0", model="llama", engine_index=str(i)).inc(8)
        c.labels(position="1", model="llama", engine_index=str(i)).inc(3)
        c.labels(position="2", model="llama", engine_index=str(i)).inc(1)
    ...
    for m in metrics:
        assert isinstance(m, Vector)
        assert m.name == "vllm:spec_decode_num_partial_accepted_tokens_per_pos"
        assert m.values == [8, 3, 1]
        assert m.labels["model"] == "llama"
        assert m.labels["engine_index"] in engine_labels
        engine_labels.remove(m.labels["engine_index"])
